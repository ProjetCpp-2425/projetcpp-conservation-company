...
void smtp::readyRead()
{
    ...
    if (state == Init && responseLine == "220") {
        *t << "EHLO localhost\r\n";
        ...
        socket->startClientEncryption(); // STARTTLS
        if (!socket->waitForEncrypted(timeout)) {
            qDebug() << "TLS encryption failed";
            state = Close;
        }
        *t << "EHLO localhost\r\n";
        t->flush();
    ...
    else if (state == Quit && responseLine == "250")
    {
        *t << "QUIT\r\n";
        ...
        // Émet le signal que le message a été envoyé.
        emit status(tr("Message sent"));
    }
    else if (state == Close)
    {
        // Fin propre de la session, mais ne ferme pas la fenêtre principale.
