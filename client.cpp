...
QSqlQueryModel* client::afficher() {
 ...
 void client::rechercher(const QString &a, QTableView *g)
  ...
  bool client::exporterPDF()
  {
      ...
      // Dimensions de la page et marges
      int leftMargin = 50;
      int topMargin = 100;
      ...
      int rowSpacing = 30; // Espacement entre les lignes
      int colSpacing = 8; // Espacement entre colonnes
      int numColumns = 8; // Nombre de colonnes
      int colWidth = (pageWidth - (numColumns - 1) * colSpacing) / numColumns;

      // Titre
      painter.setFont(QFont("Helvetica", 16, QFont::Bold));
      painter.drawText(leftMargin, currentY, pageWidth, rowHeight, Qt::AlignCenter, "Liste des Clients");
      currentY += rowHeight + 30;

      // En-tête du tableau
      painter.setFont(QFont("Helvetica", 12, QFont::Bold));
      painter.setBrush(QBrush(QColor(200, 200, 200))); // Fond gris clair pour l'en-tête
      painter.setPen(Qt::black);

      // Dessiner les colonnes de l'en-tête
      int currentX = leftMargin;
      QStringList headers = {"IDENTIFIANT", "NOM", "PRENOM", "EMAIL", "TELEPHONE","point", "promo", "UIDC"};
      for (const QString &header : headers) {
          painter.drawRect(currentX, currentY, colWidth, rowHeight);
          painter.drawText(currentX, currentY, colWidth, rowHeight, Qt::AlignCenter, header);
      ...
      // Requête SQL
      QSqlQuery query;
      query.prepare("SELECT IDENTIFIANT, NOM, PRENOM, EMAIL, TELEPHONE,POINTSF,PROMO,UIDC FROM Client");
      if (query.exec()) {
          ...
          while (query.next()) {
              ...
              QString email = query.value("EMAIL").toString();
              QString telephone = query.value("TELEPHONE").toString();
              QString point = query.value("POINTSF").toString();
              QString promo = query.value("PROMO").toString();
              QString carte = query.value("UIDC").toString();
              ...
              QStringList rowData = {identifiant, nom, prenom, email, telephone,point,promo,carte};
              for (const QString &data : rowData) {
                  painter.drawRect(currentX, currentY, colWidth, rowHeight);
                  painter.drawText(currentX, currentY, colWidth, rowHeight, Qt::AlignCenter, data);
                  currentX += colWidth + colSpacing;
              }
              currentY += rowHeight + rowSpacing; // Ajouter l'espacement entre les lignes
          }
      } else {
          qWarning() << "Erreur de requête : " << query.lastError().text();
          return false;
      ...
      yPos = pdfWriter.height() - 100;
      QFont signatureFont("Helvetica", 10);
      signatureFont.setItalic(true); // Appliquer le style italique
      painter.setFont(signatureFont);
      painter.drawText(50, yPos, "Généré par l'application Ilca");
