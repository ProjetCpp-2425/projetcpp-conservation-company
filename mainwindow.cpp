    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(socket, SIGNAL(readyRead()), this, SLOT(readyRead()));
     ...
     arduino = new Arduino(this);//permet à arduino d'avoir une durée de vie liée à celle de la fenêtre principale.

    //Connexion du signal clientFound au slot onClientFound
...
void MainWindow::onClientFound(const QString &UIDC) {
    QSqlQuery query;
    query.prepare("SELECT NOM, PRENOM, IDENTIFIANT FROM CLIENT WHERE UPPER(UIDC) = UPPER(:UIDC)");
    query.bindValue(":UIDC", UIDC);
    ...
    if (query.next()) {
        ...
        QString lcdMessage = QString("%0\n,%1\n,%2\n").arg(nom).arg(prenom).arg(id);
        qDebug() << "Message envoyé à Arduino : " << lcdMessage;
        arduino->writeData(lcdMessage.toUtf8());
    } else {
        QMessageBox::warning(this, "client introuvable", "Aucun client trouvé pour cet UIDC.");
        arduino->writeData("UNKNOWN\n");
    }
}
...
{
    client c;
    // Vérifie si le PDF a été généré avec succès
    if (c.exporterPDF()) {
        QMessageBox::information(this, "Exportation PDF", "Le fichier PDF a été créé avec succès.");
    } else {
        QMessageBox::warning(this, "Erreur", "Impossible de créer le fichier PDF.");
...
void MainWindow::on_founisseur_clicked()
{
